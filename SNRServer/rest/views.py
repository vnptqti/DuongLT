...
class FileUploadView(APIView):
    ...
    def VUS(self, x=None, SR=None, inbkpts=None, instate=None):
        print("goi vao vus!", file=sys.stderr);
        outbkpts = 0;
        outstate = 0;
        T = 0;
        ...
        ma = self.computeMA(x, SR);
        zcr = self.computeZCR(x, SR);
        lf = self.computeLF(x, SR);
        ...
        if type(instate) != type(None):
            ...
            T[0, :] = self.threshold(f1, f2);

            for i in range(0, F):
                f1 = f[U_n, i];
                f2 = f[V_n, i];
                T[i + 1] = self.threshold(f1, f2);
        else:
            T = inbkpts;

        for i in range(F):
            g[:, i] = self.normalise(f[:, i], T[i + 1])
            if i == 1:
                g[:, i] = -g[:, i];
        # Speech/Silence
        SS = self.computeSTA(f[:, 0] > T[0, 0], SR) > .5
        # Voiced/Unvoiced
        sumG = np.sum(g, axis=1)
        VU = self.computeSTA(sumG > 0, SR) > .5
    ...
    def computeMA(self, x=None, SR=None):  # Done
    ...
    def computeSTA(self, x=None, SR=None):
        N = math.floor(20e-3 * SR)
    ...
    def computeLF(self, x=None, SR=None):  # Done
        cutoff = 1000
        Wn = cutoff * 2 / SR
        [B, A] = butter(5, Wn);
        ...
        LF = self.computeSTE(xlp, SR) / self.computeSTE(x, SR)
        return LF

    def computeZCR(self, x=None, SR=None):  # Done
        N = math.floor(20e-3 * SR)
        zerosArray = np.zeros((int(N / 2), 1));
        x_pad = np.concatenate((x, zerosArray), axis=0)
    ...
    def threshold(self, f=None, g=None, fig=None):  # Done
    ...
    def normalise(self, x=None, T=None):  # Done
        n_pos = np.where(x >= T)
        n_neg = np.where(x < T)
        maxX = np.max(x)
        minX = np.min(x)
    ...
    def computeSTE(self, x=None, SR=None):  # Done
        N = math.floor(20e-3 * SR);
        zerosArray = np.zeros((int(N / 2), 1));
        x_pad = np.concatenate((x, zerosArray), axis=0)
        onesArray = np.ones(N);
    ...
    def post(self, request, *args, **kwargs):
        ...
        # xoa toan  bo file truoc khi luu
        filelist = [f for f in os.listdir('.\\media\\') if f.endswith(".wav")]
        for f in filelist:
        ...
        if file_serializer.is_valid():
            ...
            try:
                ...
                if path.exists('.\\rest\\result.out'):
                    file = open('.\\rest\\result.out', 'r')
                    T = np.zeros((4, 1));
                    ...
                    print("ko file!", file=sys.stderr);
                    inbkpts = np.array(
                        [0, 47622, 53747, 59649, 65784, 74414, 75422, 76133, 84779, 85925, 90810, 99030, 110146]);
                    instate = np.array([3, 2, 1, 2, 1, 3, 2, 1, 3, 2, 1, 3]);
                    x, SR = sf.read('.\\rest\\training.wav')
                    x = np.reshape(x, (x.shape[0], 1))
                    outbkpts, outstate, T = self.VUS(x, SR, inbkpts, instate);
                    np.savetxt('.\\rest\\result.out', T)

                if (path.isfile(os.path.join('.\\media\\', request.data['file'].name))):

                    for i in range(1, 2):
                        file = os.path.join('.\\media\\', request.data['file'].name);
                        ...
                        try:
                            ...
                            outbkpts, outstate, T = self.VUS(y, SR, T);
                            ...
                            SNR = 0;
                        rs = rs + ("\"file%s\":{\"snr\": %.2f,\"rate\": %s,\"dr\": %.2f}," % (
                            i, float(SNR), SR, (len(y) / SR)));
                ...
                #print("{\"status\": false, \"message\": null}")
                return Response({'result': 'exception'}, status=status.HTTP_201_CREATED)

            return Response({'result': SNR}, status=status.HTTP_201_CREATED)
            # return Response(file_serializer.data, status=status.HTTP_201_CREATED)
        else:
            return Response({'result': 'exception3'}, status=status.HTTP_201_CREATED)
